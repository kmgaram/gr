<?php
	...
	$sql = "select pass from gr_freeboard where num=$num";    // 레코드 검색
    ...
    if ($pass == $db_password) {        
        if ($mode=="modify")            // 수정 모드
            $url = "free_modify_form.php?num=$num";
        else                            // 삭제 모드
            $url = "free_delete.php?num=$num";
        ...
        echo "<script>
            location.href ='free_password_form.php?num=$num&error=y';
            </script>";
    }
?>
